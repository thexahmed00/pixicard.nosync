        ...
        switch ($area) {
            case self::SELLER:
                ...
                $this->configMenu = $configMenu
                    ...
                    ->filter(function ($item) use ($seller, $guard) {
                    // 🚫 If seller is not approved → only allow Profile menu
                    if (! $seller->is_approved) {
                        return $item['key'] === 'marketplace.seller.profile';
                    }

                    // ✅ Approved seller → allow all menus with permission
                    return $seller->hasPermission($item['key'], $guard);
                })
                    ->toArray();
                break;
