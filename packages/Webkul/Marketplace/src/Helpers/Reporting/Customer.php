...
class Customer extends AbstractReporting
{
    ...
    public function getPosTotalCustomersProgress($seller): array
    {
        // --- FIX IS HERE ---
        // Use the correct join to get agent IDs
        ...
        $previous = DB::table('pos_order')
            ->join('orders', 'pos_order.order_id', '=', 'orders.id')
            ->whereIn('pos_order.user_id', $agentIds) // Ensure this column name is correct
            ->whereBetween('pos_order.created_at', [$this->lastStartDate, $this->lastEndDate])
            ->distinct()
            ->count('orders.customer_id');

        // Current period calculation
        $current = DB::table('pos_order')
            ->join('orders', 'pos_order.order_id', '=', 'orders.id')
            ->whereIn('pos_order.user_id', $agentIds) // Ensure this column name is correct
            ->whereBetween('pos_order.created_at', [$this->startDate, $this->endDate])
            ->distinct()
            ->count('orders.customer_id');
